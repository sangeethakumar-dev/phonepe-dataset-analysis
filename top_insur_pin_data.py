...
#Now lets extracting data from top--> insurance-->pincode

top_insur_pin_path = "data/pulse/data/top/insurance/country/india/state/"
top_state_list=os.listdir(top_insur_pin_path)
...
for state in top_state_list:
    state_path= top_insur_pin_path+state+"/"
...
#Done extracting the  top-->insurance data -->pincode

#Now lets convert that into data frame

top_pin_insur= pd.DataFrame(col_names)

#print(top_pin_insur.head())



#So its done extracting data from top--> insurance-->pincode

#Function to return the above dataframe

def get_top_insur_pin_data():
    return top_pin_insur    
